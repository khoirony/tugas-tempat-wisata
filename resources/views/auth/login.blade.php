...
@section('body')
    ...
    <div x-data="{ success: true, error:true }" class="relative z-10 top-full">
        
        <!-- Navbar -->
        @include('components.navbar')

        <!-- Angkat Componen ke layar atas map -->
        <div class="absolute z-50 lg:left-[35%] left-3 lg:top-0 top-28 flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
            ...
            @if(session()->has('success'))
                <div x-show="success" id="alert-4" class="flex p-4 mb-2 mt-7 bg-yellow-100 rounded-lg" role="alert">
                    ...
                    <button x-on:click="success = ! success" type="button" class="ml-auto -mx-1.5 -my-1.5 bg-yellow-100 text-yellow-500 rounded-lg focus:ring-2 focus:ring-yellow-400 p-1.5 hover:bg-yellow-200 inline-flex h-8 w-8" data-dismiss-target="#alert-4" aria-label="Close">
            ...
            @if(session()->has('loginError'))
                <div x-show="error" id="alert-2" class="flex p-4 mb-2 mt-7 bg-red-400 rounded-lg" role="alert">
                    ...
                    <button x-on:click="error = ! error" type="button" class="ml-auto -mx-1.5 -my-1.5 bg-red-400 text-white rounded-lg focus:ring-2 focus:ring-red-400 p-1.5 hover:bg-red-200 inline-flex h-8 w-8" data-dismiss-target="#alert-4" aria-label="Close">
                        <span class="sr-only">Close</span>
                        <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                    </button>
                </div>
